...
interface ReviewRecord { riskScore: number; }
interface Finding {
...
export default function PrincipalDocumentPage() {
  ...
  const canBeginReview = doc.status === "SUBMITTED" || doc.status === "RESUBMITTED";
  const canResolve = doc.status === "IN_REVIEW";
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: "var(--color-bg)" }}>
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-6 py-8">
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            <button onClick={() => router.push("/principal/queue")} className="text-xs mb-1 block" style={{ color: "var(--color-text-3)" }}>
              ← Queue
            </button>
        ...
        <div className="grid grid-cols-5 gap-6">
              ...
              <div className="flex border-b" style={{ borderColor: "var(--color-border)" }}>
                ...
                >
                  ...
                  {chainVerification && (
                    ...
                    >
                      {chainVerification.valid ? "✓ Verified" : "⚠ Compromised"}
                    </span>
                  )}
                </button>
              </div>
              ...
              <div className="p-4 max-h-80 overflow-y-auto">
                {activeTab === "findings" && (
                  <div className="space-y-2">
                    {findings.length === 0 ? (
                      <p className="text-sm text-center py-4" style={{ color: "var(--color-text-3)" }}>No compliance findings recorded</p>
                    ) : (
                      findings
                        .sort((a, b) => {
                          const o = ["BLOCKER", "WARNING", "INFO"];
                              ...
                              <div className="flex-1">
                                ...
                                {selectedFinding?.id === f.id && (
                                  <div className="mt-2 space-y-1">
                                    ...
                                    <p className="text-xs font-mono" style={{ color: "var(--color-text-3)" }}>{f.citation}</p>
                                    {f.suggestedFix && (
                                      <div className="text-xs p-2 rounded mt-1" style={{ backgroundColor: "var(--color-accent-light)", color: "var(--color-accent)" }}>
                                        {f.suggestedFix}
                                      </div>
                                    )}
                                  </div>
                    ...
                    {chainVerification && (
                      ...
                      >
                        {chainVerification.valid
                          ? `✓ Audit chain verified — ${auditEvents.length} events intact`
                          : "⚠ Chain compromised: " + chainVerification.message}
                      </div>
                    )}
                      ...
                      {auditEvents.map((evt, i) => (
                        <div key={evt.id} className="flex gap-3">
                          <div className="flex flex-col items-center">
                            ...
                            {i < auditEvents.length - 1 && <div className="w-px flex-1 mt-1" style={{ backgroundColor: "var(--color-border)" }} />}
                          </div>
                          <div className="pb-4">
                            <p className="text-xs font-medium" style={{ color: "var(--color-text)" }}>
                            ...
                            {evt.note && <p className="text-xs italic mt-0.5" style={{ color: "var(--color-text-2)" }}>{evt.note}</p>}
                            <p className="text-xs font-mono mt-0.5" style={{ color: "var(--color-text-3)", fontSize: "10px" }}>
                              {evt.hash.slice(0, 16)}…
                            </p>
                          </div>
                        </div>
          ...
          <div className="col-span-2 space-y-4">
            {parsedScore !== null && (
              ...
              >
                ...
                <div className="w-full space-y-1.5">
                  {[
                    { label: "BLOCKERs", count: blockerCount, color: "var(--color-blocker)" },
                    { label: "WARNINGs", count: warningCount, color: "var(--color-warning)" },
                    { label: "INFOs", count: findings.filter(f => f.severity === "INFO").length, color: "var(--color-info)" },
            ...
            <div
              ...
              style={{ backgroundColor: "var(--color-surface)", borderColor: "var(--color-border)" }}
            >
              <p className="text-xs font-medium mb-3" style={{ color: "var(--color-text-2)" }}>Compliance Action</p>

              <textarea
                value={note}
                onChange={e => setNote(e.target.value)}
                rows={3}
                placeholder="Add a note for the advisor (required for changes requested)…"
                className="w-full px-3 py-2 rounded-md border text-xs outline-none resize-none mb-3"
              ...
              <div className="space-y-2">
                {canBeginReview && (
                  <button
                    onClick={() => doTransition("begin_review")}
                    disabled={transitioning}
                    className="w-full py-2 px-3 rounded-md text-xs font-medium border transition-colors"
                ...
                {canResolve && (
                  <>
                    <button
                      ...
                      className="w-full py-2 px-3 rounded-md text-xs font-medium text-white"
                      style={{
                        backgroundColor: "var(--color-accent)",
                        opacity: transitioning || blockerCount > 0 ? 0.5 : 1,
                      }}
                      title={blockerCount > 0 ? "Cannot approve with active BLOCKERs" : undefined}
                    >
                      {blockerCount > 0 ? `Approve (${blockerCount} blocker${blockerCount !== 1 ? "s" : ""} present)` : "Approve"}
                    </button>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        opacity: transitioning || !note.trim() ? 0.5 : 1,
                      }}
                      title={!note.trim() ? "Add a note explaining the required changes" : undefined}
                    >
                      Request Changes
                    </button>
                    ...
                    <button
                      ...
                      className="w-full py-2 px-3 rounded-md text-xs font-medium"
                      style={{
                        backgroundColor: "var(--color-blocker-light)",
                ...
                {!canBeginReview && !canResolve && (
                  <p className="text-xs text-center py-2" style={{ color: "var(--color-text-3)" }}>
                    No actions available for status: {doc.status}
                  </p>
                )}
              </div>
            </div>
...
function HighlightedContent({ content, findings, selectedFinding, onSelect }: {
  ...
  selectedFinding: Finding | null; onSelect: (f: Finding | null) => void;
}) {
